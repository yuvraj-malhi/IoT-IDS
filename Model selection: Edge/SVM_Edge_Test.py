...
Original file is located at
    https://colab.research.google.com/drive/1SNvD6TuqU2l0mUd6q5n45QpRjTTOjg7U
"""
...
for str1 in ['normal.','smurf.', 'neptune.']:
  ...
  if str1 == 'normal.':
    new = new.sample(n=150000, replace=False, random_state=1 ); ############### Change later
  else:
    new = new.sample(n=150000, replace=False, random_state=1 ); ############### Change later
...
for i in range(41):
  X_finalT.rename(columns = {i: str(i)}, inplace = True) 

X_finalT = pd.DataFrame(IE.transform(X_finalT));
X_finalT = SCALE_IN.transform(X_finalT);
Y_pred = MODEL.predict(X_finalT);
...
print("\t Test Model Speed    - "+str(avg_speed[10])+" packets/sec");
