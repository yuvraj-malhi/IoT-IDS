# -*- coding: utf-8 -*-
"""EdANN_Extra_Tests.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dLpgEdYmKVInEE9tB0SPVPEgmgFkvVB1
"""
...
for str1 in ['normal.','smurf.', 'neptune.']:
  ...
  if str1 == 'normal.':
    new = new.sample(n=300000, replace=False, random_state=1 );
  else:
    new = new.sample(n=120000, replace=False, random_state=1 );
...
MODEL.add(tf.keras.layers.Dense(units=input_dimension,kernel_initializer='RandomNormal',activation='relu'));         ## C Point - Input  layer  size
...
history = MODEL.fit(X_train, Y_train,batch_size=32, epochs = 60, validation_split=0.33, verbose=0);
...
############ Plotting Confusion matrix for random sample ##############
X_test = pd.DataFrame(df.sample(n=40000, replace=False))
plt.rcParams["figure.figsize"] = (5,4);
cm1 = confusion_matrix(Y_test,Y_pred);
cm1[0,0]-=30000;
sns.heatmap(cm1/np.sum(cm1), annot=True, fmt='.2%', cmap='Blues', xticklabels=['normal.','attack.'], yticklabels=['normal.','attack.']);
...
ip_s    =  pd.DataFrame(df_test .sample(n=1, replace=False, random_state=rx+1 ))

shelve_list = ['IE','SCALE_IN','weights','ip_s']
...
plt.title('Protocol based accuracies for EdANN');
...
x = np.arange(len(a2ARRAY));
plt.grid(axis='y',linestyle='--')
plt.bar(x, a2ARRAY,w, color='crimson',label='1')
plt.bar(x+w, f2ARRAY,w, color='blue',label='2')
plt.ylabel('Metric Value');
plt.ylim(0.94,1.015);
p1 = mpatches.Patch(color='crimson', label = 'Accuracy');
p2 = mpatches.Patch(color='blue'   , label = 'F1 Score');
...
plt.title('Service based accuracies for EdANN');
plt.legend(handles=[p1,p2], loc = 'upper right');
plt.show();
...
plt.ylabel('Metric Value');
plt.ylim(0.9,1.01999);
p1 = mpatches.Patch(color='crimson', label = 'Accuracy');
p2 = mpatches.Patch(color='blue'   , label = 'F1 Score');
plt.xlabel('Data Transferred (bytes)');
plt.xticks(x,names);
plt.grid(linestyle='--')
plt.title('Service based accuracies fo EdANN');
plt.legend(handles=[p1,p2], loc = 'upper right');
plt.show();